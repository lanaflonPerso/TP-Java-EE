    ...
    <body>
        ...
        <div>
            <form method="post" action=<c:url value="creationCommande"/> >
                <fieldset>
                	<p>Nouveau client</p>
                	<c:import url="/inc/inc_client_form.jsp" /><br/><br/>
                	<c:if test="${not empty sessionScope.sessionClients}">
                		<label for="client"> Utiliser un client existant:</label>
                		<select id="client" name="client">
                			<option value="">Choisissez un client...</option>
                			<c:forEach items="${sessionScope.sessionClients}" var="client">
                				<option value="${client.key}"><c:out value="${client.value.prenomClient}
                				${client.value.nomClient}" /></option>
                			</c:forEach>
                		</select>
                	</c:if>
				</fieldset>
